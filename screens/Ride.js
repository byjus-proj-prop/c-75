...
export default class RideScreen extends Component {
  ...
  handleTransaction = async () => {
    ...
    if (!transactionType) {
      ...
      Alert.alert("Por favor digite/digitalize um id de bicicleta válido");
    ...
    if (bikeRef.docs.length == 0) {
      ...
      bikeRef.docs.map(doc => {
        if (!doc.data().under_maintenance) {
          //se a bicicleta estiver disponível, o tipo de transação será rented (alugada)
    ...
    if (userRef.docs.length == 0) {
      ...
      userRef.docs.map(doc => {
        if (!doc.data().bike_assigned) {
          ...
          isUserEligible = false;
          Alert.alert("Termine o passeio atual para alugar outra bicicleta.");
          this.setState({
            bikeId: ""
          });
        }
